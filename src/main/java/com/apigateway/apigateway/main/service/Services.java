...
public class Services {
     ...
     * OCPOES DE PAGAMENTO, que serão
     * capturadas na interação com o usuário em `MenuDeOpcoes`.
     */
    public void transactionRequest(String command, float v, String pm, String pt, String pmst)
        ...
        HttpRequest request = HttpRequest.newBuilder()
                ...
                .POST(HttpRequest.BodyPublishers.ofString(body.bodyRequest(command, v, pm, pt, pmst)))
                .build();
        System.out.println(body.bodyRequest(command, v, pm, pt, pmst));
        HttpResponse<String> response = client.send(request, HttpResponse.BodyHandlers.ofString());

        /*
         * System.out.
         * println("Resposta da Payer para o callback após receber a requisição enviada\n"
         * +
         * response.body() + "\n"); //debug
         */
    }
    ...
    /**
     * Metodo para realizar a requisição de cancelamento de pagamento. Ela não leva
     * parâmetros.
     * O valor usado para identificar o pagamento que será cancelado é o id da
     * transação. O metodo
     * que captura esse valor é chamado em `CallbackController`, e atribuído
     * diretamente no body
     ...
     * System.out.
     * println("Requisição que está sendo da enviada da automação para a Payer\n" +
